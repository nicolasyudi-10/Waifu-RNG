...
const UI = {
  ...
  playRoll(result, cooldownMs) {
    if (this.pending) this.commit(this.pending);   // garante que o giro anterior foi contabilizado
    this.pending = result;

    const token = ++this.animToken;
    const e = this.els;
    e.card.classList.remove('is-empty', 'pop', 'shake');
    e.badge.hidden = true;

    const duration = this.reduceMotion ? 0 : Math.min(CONFIG.revealMaxMs, cooldownMs * 0.6);
    const start = performance.now();

    const finish = () => {
      if (token === this.animToken) {
        e.card.classList.remove('is-rolling');
        this.showWaifu(result.waifu, result.coins);
        e.badge.hidden = !result.isNew;
        void e.card.offsetWidth;                    // reinicia a animação CSS
        e.card.classList.add('pop');

        const idx = RARITY_INDEX[result.rarity.id];
        if (idx >= CONFIG.hotRarityIndex) this.flash(result.rarity);
        if (idx >= CONFIG.shakeRarityIndex) { void e.card.offsetWidth; e.card.classList.add('shake'); }
      }
      this.commit(result);
    };
...
const Game = {
  ...
  tick() {
    ...
    if (this.auto && remaining === 0) this.roll();
  },
